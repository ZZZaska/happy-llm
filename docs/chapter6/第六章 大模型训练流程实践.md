...
training_args = TrainingArguments(
    ...
    per_device_train_batch_size=4,# 单卡训练的 batch_size
    gradient_accumulation_steps=4,# 梯度累计步数，实际 bs = 设置的 bs * 累计步数
    logging_steps=10,# 打印 loss 的步数间隔
    num_train_epochs=1,# 全量数据集跑一遍的训练轮数
    save_steps=100, # 保存模型权重的步数间隔
    learning_rate=1e-4,# 学习率
    gradient_checkpointing=True# 开启梯度检查点
)
```
- `batch`: 一次喂给模型的样本数量 （`block` 一条样本的长度 / 序列长度）
- `gradient_accumulation_steps`: 如果显卡太弱，那么`batch_size` 只能设为 1，但大模型需要大 Batch 才能稳定学习, 多次误差（梯度）累计后统一更新模型。
- 真实 Batch Size = 单卡 Batch Size $\times$ 累加步数 $\times$ 显卡数量
- `gradient_checkpointing=True`： 梯度检查点（时间换空间）。前向传播保存的中间激活值（反向传播需要用它们算梯度）炸显存，所以启动梯度检查点=不保存中间激活值，需要用的时候再重新计算一遍前向传播。
...
trainer = Trainer(
    model=model,
    args=training_args,
    train_dataset= IterableWrapper(train_dataset), # 流式包装器，防止内存爆炸
    eval_dataset= None,
    tokenizer=tokenizer, 
...
```
- `data_collator=default_data_collator`: 使用前提（block_size 固定，不需要动态 padding）

再使用 train 方法，即会按照配置好的训练超参进行训练和保存（自动自动把模型分发到多张显卡上，自动调度 DeepSpeed 优化器，自动从硬盘读取数据，自动算梯度，自动打印 Loss 进度条...）：
...
1. 空间管理：--save_total_limit 1 这个设置非常聪明！它保证了不管保存多少次 checkpoint，硬盘里永远只留最新的一份，这对于你现在只有 16GB 的剩余空间来说是“保命神技”。

3. 显存优化组合拳：--bf16 + --gradient_checkpointing + --deepspeed ./ds_config_zero2.json，这是极其标准的满血训练三板斧。
